...
async def respond_to_text(update: Update, user_id: int, text: str):
    ...
    if match:
        ...
        await update.message.reply_text(
            f"\U0001F4C1 Siniestro activado: {match.group(1)}"
        )
        return
    ...
    try:
        ...
        if r.status_code != 200 or not r.text.strip():
            ...
            json_data = r.json()
            respuesta = (
                json_data.get("textResponse") or "\U0001F916 No encontr\u00E9 respuesta."
            )
    except Exception as e:
        respuesta = f"\u274C Error: {e}"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await respond_to_text(update, update.effective_user.id, update.message.text)
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await respond_to_text(update, update.effective_user.id, text)
